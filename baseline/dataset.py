...
if __name__ == "__main__":
   dataset =  DRDataset(
      images_folder="../data/train/images_resized_650/",
      path_to_csv="../data/train/trainLabels.csv",
      transform=config.train_transforms,
   )

   loader = DataLoader(
      dataset=dataset, batch_size=config.BATCH_SIZE, num_workers=config.NUM_WORKERS, shuffle=True, pin_memory=config.PIN_MEMORY
   )
